...
<%@page import="com.eib.projetop1.beans.Professor"%>
<%@page import="com.eib.projetop1.beans.Curso"%>
                ...
                <div class="col-lg-12">
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <form role="form" action="controller.do?command=alterarTurma" method="post">
                                <fieldset class="col-lg-8">
										  <div class="form-row">
										    <div class="form-group col-md-6">
										    	<input type="hidden" name="id" id="id" value="${param.id}" class="form-control" readonly required>
										    	<label id="idProfessor">Professor</label>
											      <select name="idProfessor" id="idProfessor" class="form-control" required >
													<option value="">Selecione um Professor</option>
												    <%
														Professor p1 = new Professor();
														Collection<Professor> lista = p1.buscarTodos();
			
														for (Professor x : lista) {
											        %>
											           <option value="<%=x.getId_professor() %>"><%=x.getNome() %></option>
											        <% 
											        	} 
													%>
												</select>
										    ...
										    <div class="form-group col-md-6">
										      <label id="idCurso">Curso</label>
											      <select name="idCurso" id="idCurso" class="form-control" required >
													<option value="">Selecione um Curso</option>
												    <%
														Curso c1 = new Curso();
														Collection<Curso> lista2 = c1.buscarTodos();
			
														for (Curso x : lista2) {
											        %>
											           <option value="<%=x.getId_curso() %>"><%=x.getTitulo() %></option>
											        <% 
											        	} 
													%>
												</select>
										    ...
										    <div class="form-group col-md-12">
										      ...
										      <input type="number" value="40" max="150" min="0" class="form-control" name= "qtdhoras" id="qtdhoras" placeholder="Qtd.Horas" required>
										    </div>
										  <div class="form-row">
										    <div class="form-group col-md-4">
										      <label for="datainicio">Data Início</label>
										      <input type="date" class="form-control" name= "datainicio" id="datainicio" pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}" required>
										    </div>
										    <div class="form-group col-md-6">
										      <label for="datafim">Data Fim</label>
										      <input type="date" class="form-control" name="datafim" id="datafim" pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}" required>
										    </div>
										  </div>
										  <div class="checkbox col-md-12">
												<label> <input name="ativo" type="checkbox" 
													value="${param.ativo}"  ${param.ativo == true ? "checked" : ""}>Ativo
												</label>
											</div>
                                    <!-- Change this to a button or input when using this as a form -->
                                    <div class="form-group col-md-12">
                                    	<button type="submit" class="btn btn-outline btn-warning">Alterar</button>
                                    </div>
                                </fieldset>
                            </form>
                        </div>
